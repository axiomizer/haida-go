...
class TestNN(unittest.TestCase):
    ...
    def test_conv(self):
        ...
        optimizer = torch.optim.SGD(torch_conv.parameters(), lr=hp.LEARNING_RATE, weight_decay=hp.WEIGHT_DECAY*2)
        optimizer.zero_grad()
    ...
    def test_res(self):
        ...
        np_target = np.random.randn(self.minibatch_size, self.filters, self.board_size, self.board_size)
        loss = torch.nn.MSELoss(reduction='mean')
        optimizer = torch.optim.SGD(torch_res.parameters(), lr=hp.LEARNING_RATE, weight_decay=hp.WEIGHT_DECAY*2)
        optimizer.zero_grad()
        loss(torch_results, torch.tensor(np_target, dtype=torch.float64)).backward()
        optimizer.step()
    ...
    def test_pol(self):
        ...
        optimizer = torch.optim.SGD(torch_pol.parameters(), lr=hp.LEARNING_RATE, weight_decay=hp.WEIGHT_DECAY*2)
        optimizer.zero_grad()
    ...
    def test_val(self):
        ...
        loss = torch.nn.MSELoss(reduction='mean')
        optimizer = torch.optim.SGD(torch_val.parameters(), lr=hp.LEARNING_RATE, weight_decay=hp.WEIGHT_DECAY*2)
        optimizer.zero_grad()
        loss(torch_results, target).backward()
        optimizer.step()
    ...
    def test_full_net(self):
        ...
        optimizer = torch.optim.SGD(torch_net.parameters(), lr=hp.LEARNING_RATE, weight_decay=hp.WEIGHT_DECAY*2)
        optimizer.zero_grad()
